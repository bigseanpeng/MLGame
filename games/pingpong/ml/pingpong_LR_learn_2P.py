...
for item in folder_content:
        if os.path.isdir(folder_path + '\\' + item):
               ...
               data_list = data_list["ml_2P"]["scene_info"] #將資料夾中檔案的必要場警資訊全部匯入data List
               game_file.close()
               """
               try:
                   os.remove(file_name)
...
#--------------------------------------------------------------------------------
        ...
        for i in range (0,len(data_list)):
            ...
            if data_list[i]['ball'][1]<80+1 and data_list[i]['ball_speed'][0] > 0 and data_list[i]['status'] == "GAME_ALIVE":
            ...
            elif data_list[i]['ball'][1]<80+1 and data_list[i]['ball_speed'][0] < 0 and data_list[i]['status'] == "GAME_ALIVE":
